...
def create_labelstudio_project(dataset, dataset_name, class_names):
    """
    Automatically create a Label Studio project and upload all samples using FiftyOne's annotation API.

    This uploads all dataset samples to a single Label Studio project in batches.
    Uses FiftyOne's annotate() API with proper configuration to ensure all batches
    go to the same project.

    If a project with the same name already exists, it will be deleted and recreated.
    """
    # Check if Label Studio is configured
    labelstudio_url = os.getenv('FIFTYONE_LABELSTUDIO_URL')
    if not labelstudio_url:
        return  # Label Studio not configured, skip

    print(f"\n{'='*60}")
    print("Label Studio Integration: Creating project...")
    print(f"{'='*60}")

    try:
        api_key = os.getenv('FIFTYONE_LABELSTUDIO_API_KEY')
        if not api_key:
            print("Warning: FIFTYONE_LABELSTUDIO_API_KEY not set, skipping Label Studio integration")
            print(f"{'='*60}\n")
            return
        ...
        print(f"Project name: {project_name}")
        print(f"Label Studio server: {labelstudio_url}")
        print(f"Classes: {', '.join(class_names)}")

        # Configure FiftyOne's Label Studio annotation backend
        ...
        try:
            ...
            print(f"Checking for existing Label Studio project...")
            ls_client = Client(url=labelstudio_url, api_key=api_key)
            projects = ls_client.get_projects()
            existing_project = None
            for project in projects:
                if project.get_params().get('title') == project_name:
                    existing_project = project
                    break

            if existing_project:
                project_id = existing_project.get_params().get('id')
                print(f"⚠ Found existing project '{project_name}' (ID: {project_id})")
                print(f"⚠ Deleting existing project...")
                ls_client.delete_project(project_id)
                print(f"✓ Existing project deleted successfully")
            else:
                print(f"✓ No existing project found")

        except ImportError:
            print("Warning: label-studio-sdk not installed, skipping existence check")
        except Exception as e:
            print(f"Warning: Could not check/delete existing project: {e}")

        # Upload all samples to Label Studio in batches
        ...
        batch_size = int(os.getenv('LABELSTUDIO_BATCH_SIZE', '90'))  # Keep below 100 to avoid LS limit
        num_batches = (total_samples + batch_size - 1) // batch_size

        print(f"\nUploading {total_samples} samples to Label Studio in {num_batches} batches...")
        ...
        # Upload all batches to the same project using FiftyOne's annotate() API
        for batch_num in range(num_batches):
            start_idx = batch_num * batch_size
            end_idx = min((batch_num + 1) * batch_size, total_samples)
            batch_count = end_idx - start_idx

            print(f"Batch {batch_num + 1}/{num_batches}: Uploading samples {start_idx + 1} to {end_idx}...")

            try:
                # Create a view for this batch
                batch_view = dataset.skip(start_idx).limit(batch_count)
                anno_key = f"labelstudio_batch_{batch_num}_{dataset_name}"

                # Upload batch using FiftyOne's annotate() API
                # Using the same project_name ensures all batches go to the same project
                batch_view.annotate(
                    anno_key,
                    backend="labelstudio",
                    label_field="ground_truth",
                ...
                progress_pct = (uploaded_count / total_samples) * 100
                print(f"  ✓ Batch {batch_num + 1} completed: {batch_count} samples uploaded")
                print(f"  Progress: {uploaded_count}/{total_samples} ({progress_pct:.1f}%)")

                # Clean up the batch annotation run to avoid clutter
                try:
                    dataset.delete_annotation_run(anno_key)
                except:
                    pass
        ...
        # Summary
        print(f"{'='*60}")
        if uploaded_count == total_samples:
            print(f"✓ Upload complete!")
            print(f"✓ Successfully uploaded all {total_samples} samples to Label Studio")
        else:
            print(f"⚠ Partial upload completed")
            print(f"✓ Successfully uploaded {uploaded_count}/{total_samples} samples")
            if failed_batches:
                print(f"✗ Failed batches: {', '.join(map(str, failed_batches))}")

        print(f"✓ Label Studio project: {project_name}")
        print(f"✓ Start annotating at: {labelstudio_url}")
        print(f"{'='*60}\n")
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # 3.6. Create Label Studio project if Label Studio sync is enabled
    # ------------------------------------------------------------------
    create_labelstudio_project(dataset, db_name, names)
v
    # ------------------------------------------------------------------
